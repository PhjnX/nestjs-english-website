...
export class ReadingService {
  ...
  async deleteTest(id: number) {
    ...
    // 1. Tìm tất cả part thuộc test này
    const parts = await this.prisma.reading_part.findMany({
      where: { reading_test_id: id },
      select: { id: true },
    });
    const partIds = parts.map((p) => p.id);

    // 2. Tìm tất cả group thuộc các part này
    const groups = await this.prisma.reading_group.findMany({
      where: { reading_part_id: { in: partIds.length ? partIds : [0] } },
      select: { id: true },
    });
    const groupIds = groups.map((g) => g.id);

    // 3. Xoá hết question của các group này
    if (groupIds.length > 0) {
      await this.prisma.reading_question.deleteMany({
        where: { group_id: { in: groupIds } },
      });
      await this.prisma.reading_group.deleteMany({
        where: { id: { in: groupIds } },
      });
    }
    // 4. Xoá hết part
    if (partIds.length > 0) {
      await this.prisma.reading_part.deleteMany({
        where: { id: { in: partIds } },
      });
    }
    // 5. Cuối cùng xoá test
  ...
  async deletePart(id: number) {
    // --- Manual cascade delete ---
    // 1. Tìm tất cả group thuộc part này
    const groups = await this.prisma.reading_group.findMany({
      where: { reading_part_id: id },
      select: { id: true },
    });
    const groupIds = groups.map((g) => g.id);

    // 2. Xoá hết question của các group này
    if (groupIds.length > 0) {
      await this.prisma.reading_question.deleteMany({
        where: { group_id: { in: groupIds } },
      });
    ...
    // 3. Cuối cùng xoá part
    return this.prisma.reading_part.delete({
      where: { id },
    });
  }
  ...
  async deleteGroup(id: number) {
    // --- Manual cascade delete ---
    // 1. Xoá hết question thuộc group này
    await this.prisma.reading_question.deleteMany({
      where: { group_id: id },
    });
    // 2. Xoá group
    return this.prisma.reading_group.delete({
      where: { id },
    });
  }
